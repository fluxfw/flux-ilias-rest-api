...
namespace FluxIliasRestApi\Service\UserRole;
...
trait UserRoleQuery
{
    ...
    private function getUserRoleQuery(?int $user_id = null, ?string $user_import_id = null, ?int $role_id = null, ?string $role_import_id = null) : string
    {
        ...
        if ($role_id !== null) {
            $wheres[] = "object_data.obj_id=" . $this->ilias_database->quote($role_id, ilDBConstants::T_INTEGER);
        }

        if ($role_import_id !== null) {
            $wheres[] = "object_data.import_id=" . $this->ilias_database->quote($role_import_id, ilDBConstants::T_TEXT);
        }
